...
import {
  EstrazioneDatiPersonaleMezziModel,
  Personale,
  ScadenzePersonale,
} from "app/models/personale.model";
...
@Component({
  ...
  styleUrls: [
    "./personale-dashboard.component.scss",
...
export class PersonaleDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.idPersonale = Number.parseInt(
    ...
    //     fullScreen: true
    //   });
    this.personaleservice.getPersonale(this.idPersonale).subscribe(
      (res) => {
      ...
      () => {
        this.nuovopersonale.fullName =
          this.nuovopersonale.name + " " + this.nuovopersonale.surname;
        document.getElementById("cards").click();
        this.imageSource = this._sanitizer.bypassSecurityTrustResourceUrl(
          this.nuovopersonale.file as string
        );
        // estrapolo le skills
        let sk = "";
    ...
    this.personaleservice.getCantieriPersonale(this.idPersonale).subscribe(
      (res) => {
        LoggingService.log(
          "cantieri in cui lavora estratti",
          LogLevel.debug,
          res
        );
      ...
      (err) => {
        LoggingService.log(
          "ERRORE get cantieri in cui lavora ",
          LogLevel.error,
          err
        );
        this._spinner.hide();
      ...
      () => {
        ...
        this.cantieriPersonale.forEach((cantiere) => {
          if (cantiere.state != "Terminato") {
            ...
            const index = cantieriCorretti.findIndex(
              (e) => e.id == cantiere.id
            );
            if (index == -1) {
        ...
        // this.cantieriPersonale = cantieriCorretti;
        LoggingService.log(
          "cantieri in cui lavora",
          LogLevel.debug,
          this.cantieriPersonale
        );
      }
    );
    // escludo dalle scadenze imminenti quelle con stato Chiuso(mettendo a false il secondo parametro)
    this.personaleservice
      .getScadenzePersonaleFiltered(this.idPersonale, false, null, 30)
      .subscribe(
        (res: ScadenzePersonale[]) => {
          LoggingService.log(
            "Scadenze filtrate 30 giorni ottenute",
            LogLevel.debug,
            res
          );
        ...
        (err) => {
          LoggingService.log(
            "ERRORE get scadenze filtrate 30 giorni",
            LogLevel.error,
            err
          );
          this._spinner.hide();
    ...
    // escludo dalle scadenze imminenti quelle con stato Chiuso(mettendo a false il secondo parametro)
    this.personaleservice
      .getScadenzePersonaleFiltered(this.idPersonale, false, null, 15)
      .subscribe(
        (res: ScadenzePersonale[]) => {
          LoggingService.log(
            "Scadenze filtrate 15 giorni ottenute",
            LogLevel.debug,
            res
          );
        ...
        (err) => {
          LoggingService.log(
            "ERRORE get scadenze filtrate 15 giorni",
            LogLevel.error,
            err
          );
          this._spinner.hide();
    ...
    // escludo dalle scadenze imminenti quelle con stato Chiuso(mettendo a false il secondo parametro)
    this.personaleservice
      .getScadenzePersonaleFiltered(this.idPersonale, false, null, 0)
      .subscribe(
        (res: ScadenzePersonale[]) => {
          LoggingService.log(
            "Scadenze filtrate 0 giorni ottenute",
            LogLevel.debug,
            res
          );
        ...
        (err) => {
          LoggingService.log(
            "ERRORE get scadenze filtrate 0 giorni",
            LogLevel.error,
            err
          );
          this._spinner.hide();
    ...
    const idIntervallo = setInterval(() => {
      if (
        this.scadenzePersonale0 != null &&
        this.scadenzePersonale15 != null &&
        this.scadenzePersonale30 != null
      ) {
  ...
  filterscadenze() {
    ...
    console.log("ce l'abbiamo fatta", [
      this.scadenzePersonale0,
      this.scadenzePersonale15,
      this.scadenzePersonale30,
    ]);
    document.getElementById("cardScadenze").click();
  }
    ...
    switch (binding) {
      case "datanascita": {
        ...
        if (
          ...
          this.nuovopersonale.birthday =
            event.year + "/" + (event.month + 1).toString() + "/" + event.day;
        }
        LoggingService.log(
          "la data di nascita modificata è",
          LogLevel.debug,
          this.nuovopersonale.birthday
        );
        break;
      ...
      case "scadenzaAssunzione": {
        LoggingService.log(
          "la data di scadenza assunzione è",
          LogLevel.debug,
          event
        );
        break;
      ...
      case "idoneitaSanitaria": {
        LoggingService.log(
          "la data di Idoneità sanitaria hse è",
          LogLevel.debug,
          event
        );
        break;
      ...
      case "orelavoratedatainizio": {
        LoggingService.log(
          "la data di inizio ore lavorate è",
          LogLevel.debug,
          event
        );
        if (event.year == null && event.month == null && event.day == null) {
      ...
      case "orelavoratedatafine": {
        ...
        LoggingService.log(
          "la data di fine ore lavorate è",
          LogLevel.debug,
          event
        );
        const data = new Date(
  ...
  getOreLavorate(dataInizio: string, datafine: string, button?: string) {
    ...
    if (this.dataInizioOreLavorate.split("-")[2].length == 1) {
      this.dataInizioOreLavorate =
        this.dataInizioOreLavorate.substring(0, 8) +
        "0" +
        this.dataInizioOreLavorate.substring(8, 10);
    }
    if (this.dataFineOreLavorate.split("-")[2].length == 1) {
      this.dataFineOreLavorate =
        this.dataFineOreLavorate.substring(0, 8) +
        "0" +
        this.dataFineOreLavorate.substring(8, 10);
    }
    console.log(this.dataInizioOreLavorate, this.dataFineOreLavorate);

    this.personaleservice
      .getTimeCardsPersonale(
        this.idPersonale,
        this.dataInizioOreLavorate,
        this.dataFineOreLavorate
      )
  ...
  addImageToPersonale(event) {
    ...
    if (files) {
      ...
      LoggingService.log(
        "formdata",
        LogLevel.debug,
        this.formData.getAll("formFiles")
      );
    }
  ...
  updatePersonale() {
    ...
    this.skills.forEach((element) => {
      ...
      this.nuovopersonale.skills = this.nuovopersonale.skills.concat(
        element + "$"
      );
    });
    this._spinner.show(undefined, {
    ...
    LoggingService.log(
      "personale che sto per aggiornare",
      LogLevel.debug,
      this.nuovopersonale
    );

    //se non è abilitato ad essere un utente, elimino il ruolo
    if (!this.nuovopersonale.canLogin) this.nuovopersonale.role = null;
    //se è abilitato ad essere un utente, se non è stato selezionato un ruolo, lo imposto di default a 'foreman'
    else if (
      this.nuovopersonale.canLogin &&
      (!this.nuovopersonale.role || this.nuovopersonale.role == "")
    )
      this.nuovopersonale.role = "Foreman";

    this.personaleservice
      .updatePersonale(this.nuovopersonale, this.nuovopersonale.canLogin)
      .subscribe(
        (res) => {
  ...
  checkEmail() {
    ...
    this.personaleservice.emailExists(this.nuovopersonale.email).subscribe(
      (res) => {
        LoggingService.log("email esistente", LogLevel.debug, res);
        if (res) this.disableConfirmButton = true;
  ...
  resendResetEmail() {
    this.authService
      .forgotPassword({ username: this.personale.email })
      .subscribe(
        (res) => {
          if (res.ok) {
  ...
  estraiDettagli(exportType: "pdf" | "xlsx") {
    ...
    this.personaleservice
      .estraiDatiPersonale(
        [this.idPersonale],
        exportType,
        this.estraiDatiModel.dateFrom,
        this.estraiDatiModel.dateTo
      )
      .subscribe(
