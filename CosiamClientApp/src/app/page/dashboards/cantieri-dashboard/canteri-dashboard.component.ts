...
import { AvanzamentoTemporaleCantiere, Cantiere, DatiFinanziari } from 'app/models/cantiere.model';
...
import { TableColumn, TableDataRow } from 'app/shared/components/editable-grid-mini/editable-grid-mini.component';
...
const dataArray: TableDataRow[] = [
  {
    'COSTI': { title: 'titolo1.1', value: '1.11', format: 'number', titleClasses: 'text-success' },
    'RICAVI': { title: 'titolo1.2', value: '1,24', format: 'string', titleClasses: 'text-primary' },
    'UTILE': { title: 'titolo1.3', value: '2,10', format: 'currency', titleClasses: 'text-primary' },
  },
  {
    'COSTI': { title: 'titolo2.1', value: '2,10', format: 'currency', readonly: true },
    'RICAVI': { title: 'titolo2.2', value: '2.2', format: 'string', titleClasses: 'text-danger' },
    'UTILE': { title: 'titolo2.2', value: '2.2', format: 'string', titleClasses: 'text-danger' }
  },
  {
    'colonna1': {
      title: 'titolo3.1',
...
export class CanteriDashboardComponent implements OnInit {
  ...
  model: Cantiere;
  releated = [{ id: 1, desc: 'desc1' }, { id: 2, desc: 'desc2' }, { id: 3, desc: 'desc3' }];
  ...
  constructor(private contrattoservice: ContrattoService, private _servicepersonale: PersonaleService, private _modalService: NgbModal, private sanitizer: DomSanitizer, private activatedRoute: ActivatedRoute, private cantieriserv: CantieriService, private clientiservice: ClientiService, private _datiFinanziariService: FinancialCardService) {
  ...
  ngOnInit(): void {
    ...
    this.cantieriserv.getCantiereFullSpecification(this.idCantiere)
      .subscribe(
        (res) => {
          this.model = res;
        ...
        (err) => {
          ...
          LoggingService.log('errore nel recupero del cantiere', LogLevel.error, err)
        },
        () => {

          this.descrizionelavori = this.model.description;

          this.contrattoservice.getContratto(this.model.idContratto)
            .subscribe(
              (res) => {
                LoggingService.log('contratto relativo al cantiere ottenuto', LogLevel.debug, res);
                ...
                this.clientiservice.getCliente(this.contrattoCantiere.idCliente)
                  .subscribe(
                    (res) => {
                    ...
                    (err) => {
                      LoggingService.log('errore get clienti ', LogLevel.error, err)
                    },
                    () => {
                      // imposto il logo sulla dashboard, preso dal cliente
                      LoggingService.log('il logo del cliente è ', LogLevel.warn, this.clienteCantiere)
                      if (this.clienteCantiere.file)
                        this.imageSource = this.sanitizer.bypassSecurityTrustResourceUrl(this.clienteCantiere.file as string);
          ...
          this.cantieriserv.getPMsCantiere(this.idCantiere)
            .subscribe(
              (res: Personale[]) => {
          ...
          this.cantieriserv.getForemenCantiere(this.idCantiere)
            .subscribe(
              (res) => {
    ...
    this.cantieriserv.getNoteDiCantiere(this.idCantiere)
      .subscribe(
        (res) => {
          LoggingService.log('note per questo cantiere', LogLevel.debug, res);
          this.relatedNotes = res;
          for (const n of this.relatedNotes) {
            n.nota.creationDate = new Date(Date.parse(n.nota.creationDate.toString()));
          }
          ...
          LoggingService.log('errore nella get delle note per questo cantiere', LogLevel.debug, err);
        }
      );
    ...
    this._servicepersonale.getAllPersonale()
      .subscribe(
        (res) => {
  ...
  onFileSelected(event: _File) {
    ...
    LoggingService.log('File recuperato dal componente di upload:', LogLevel.debug, event);
    if (event.type.includes('image')) {
      this.cantieriserv.getSingleImmagineCantiere(this.idCantiere, event.id)
        .subscribe(
          (res) => {
  ...
  async scaricameteo(cap: number) {
    try {
      ...
      if (x) {
        ...
        (this.meteo.today as unknown as Number) = Math.round(((y.daily[0].temp.min) + (y.daily[0].temp.max)) / 2);
        (this.meteo.tomorrow as unknown as Number) = Math.round(((y.daily[1].temp.min) + (y.daily[1].temp.max)) / 2);
        (this.meteo.dayaftertomor as unknown as Number) = Math.round(((y.daily[2].temp.min) + (y.daily[2].temp.max)) / 2);
  ...
  updateProxSal() {
    ...
    if (this.nuoviDatiFinanziariCantiere.proxSal.toString().includes(',')) {
      this.nuoviDatiFinanziariCantiere.proxSal = this.nuoviDatiFinanziariCantiere.proxSal.toString().replace(',', '.');
    }

    let proxSal = this.nuoviDatiFinanziariCantiere.proxSal.toString();
    proxSal = Number.parseFloat(proxSal).toFixed(2);

    this.nuoviDatiFinanziariCantiere = JSON.parse(JSON.stringify(this.datiFinanziariCantiere));
    this.nuoviDatiFinanziariCantiere.proxSal = proxSal;

    this._datiFinanziariService.postFinancialCard(this.nuoviDatiFinanziariCantiere).subscribe(
      (res) => {
        console.log('aggiornato proxSal', res);
        this.datiFinanziariCantiere = JSON.parse(JSON.stringify(this.nuoviDatiFinanziariCantiere));
        this.datiFinanziariCantiere = { ...this.datiFinanziariCantiere };

        if (this.valoreDellaProduzione.toString().includes(',')) {
          (this.valoreDellaProduzione as unknown as string) = this.valoreDellaProduzione.toString().replace(',', '.');
        }
        this.mancanti = Number.parseFloat(this.datiFinanziariCantiere.proxSal) - Number.parseFloat(this.valoreDellaProduzione.toString());
  ...
  updateCantiere() {
    ...
    LoggingService.log('Sto per fare l\'update del cantiere inserendo questi dati ', LogLevel.debug, this.nuovocant);
    this.cantieriserv.updateCantiere(this.nuovocant)
      .subscribe(
        (res) => {
        ...
        () => {
          this.cantieriserv.updateForemenCantiere(this.nuovocant.id, this.capocantieri_selezionati)
            .subscribe(
              (res) => {
                LoggingService.log('update capocantiere andato', LogLevel.debug, res);
                this.capoCantieri = this.personale_get.filter(x => this.capocantieri_selezionati.includes(x.id));
              },
              (err) => {
                LoggingService.log('errore update capocantiere ', LogLevel.error, err);
              },
              () => {
                this.capocantieri_selezionati = null;
              },
            )
          this.cantieriserv.updatePMCantiere(this.nuovocant.id, this.pm_selezionati)
            .subscribe(
              (res) => {
                LoggingService.log('update PM cantiere andato', LogLevel.debug, res);
                this.pmsCantiere = this.personale_get.filter(x => this.pm_selezionati.includes(x.id));
              },
              (err) => {
                LoggingService.log('errore update PM cantiere ', LogLevel.error, err);
              },
              () => {
                this.pm_selezionati = [];
              },
  ...
  cambia(event, binding: string, otherData: any = null) {
    ...
    switch (binding) {
      case 'datainizio': {
        this.nuovocant.start = new Date(event.year, event.month, event.day, ore, minuti, secondi);
        LoggingService.log('la data di inizio del nuovo cantiere è', LogLevel.debug, this.nuovocant.start);
        break;
      }
      case 'dataFineStimata': {
        this.nuovocant.estimatedEnding = new Date(event.year, event.month, event.day, ore, minuti, secondi);
        LoggingService.log('la data di fine del nuovo cantiere è', LogLevel.debug, this.nuovocant.estimatedEnding);
        break;
      }
      case 'budgetdatainizio': {
        LoggingService.log('la data di inizio del nuovo cantiere è', LogLevel.debug, event);
        break;
      }
      case 'budgetdatafine': {
        LoggingService.log('la data di fine impostata nel budget è', LogLevel.debug, event);
        break;
      }
      case 'produzionemensiledatainizio': {
        LoggingService.log('la data di inizio impostata nel dettaglio produzione mensile è', LogLevel.debug, event);
        break;
      }
      case 'produzionemensiledatafine': {
        LoggingService.log('la data di fine impostata nel dettaglio produzione mensile è', LogLevel.debug, event);
        break;
      }
      case 'datacantiere': {
  ...
  validationModifyForm(form) {
    ...
    if (this.nuovocant.workBudget == null && this.nuovocant.chargesBudget == null) {
      this.nuovocant.orderAmount = null;
    }
    this.nuovocant.orderAmount = (this.nuovocant.workBudget ?? 0) + (this.nuovocant.chargesBudget ?? 0);

    // 2 cifre decimali
    if (this.nuovocant.orderAmount)
      this.nuovocant.orderAmount = Number.parseFloat(this.nuovocant.orderAmount.toFixed(2));
    ...
    this.nuovocant.finalAmount = this.nuovocant.totalGrossWorkAmount + this.nuovocant.totalChargesAmount
    ...
    if (this.nuovocant.finalAmount)
      this.nuovocant.finalAmount = Number.parseFloat(this.nuovocant.finalAmount.toFixed(2));
  ...
  confermaDatiFinanziari() {
    ...
    (this.nuoviDatiFinanziariCantiere.creditiVsClienti as unknown as string) = this.nuoviDatiFinanziariCantiere?.creditiVsClienti?.toString() ?? this.datiFinanziariCantiere?.creditiVsClienti.toString();
    (this.nuoviDatiFinanziariCantiere.debitiVsFornitori as unknown as string) = this.nuoviDatiFinanziariCantiere?.debitiVsFornitori?.toString() ?? this.datiFinanziariCantiere?.debitiVsFornitori.toString();
    (this.nuoviDatiFinanziariCantiere.saldo as unknown as string) = this.nuoviDatiFinanziariCantiere?.saldo?.toString() ?? this.datiFinanziariCantiere?.saldo.toString();
    if (this.nuoviDatiFinanziariCantiere.cashflow)
      this.nuoviDatiFinanziariCantiere.cashflow = Math.round(this.nuoviDatiFinanziariCantiere.cashflow * 100) / 100; //arrotondo a due cifre decimali
    this._datiFinanziariService.postFinancialCard(this.nuoviDatiFinanziariCantiere).subscribe(
      (res) => {
  ...
  validationForm(form) {
    ...
    for (const [key, value] of Object.entries(this.nuoviDatiFinanziariCantiere)) {
      console.log(key, value);
      if ((key == 'daContabilizzare' || key == 'daFatturare' || key == 'daIncassare') && parseFloat(value)) {
        this.nuoviDatiFinanziariCantiere.creditiVsClienti += parseFloat(value);
        this.nuoviDatiFinanziariCantiere.creditiVsClienti = parseFloat(this.nuoviDatiFinanziariCantiere.creditiVsClienti.toFixed(3)); // 3 cifre decimali
    ...
    for (const [key, value] of Object.entries(this.nuoviDatiFinanziariCantiere)) {
      console.log(key, value);
      if ((key == 'paghe' || key == 'fattureRicevute' || key == 'fattureDaRicevere') && parseFloat(value)) {
        this.nuoviDatiFinanziariCantiere.debitiVsFornitori += parseFloat(value);
        this.nuoviDatiFinanziariCantiere.debitiVsFornitori = parseFloat(this.nuoviDatiFinanziariCantiere.debitiVsFornitori.toFixed(3)); // 3 cifre decimali
    ...
    for (const [key, value] of Object.entries(this.nuoviDatiFinanziariCantiere)) {
      console.log(key, value);
      if ((key == 'debitiABreve' || key == 'anticipazioniDaRestituire') && parseFloat(value)) {
        this.nuoviDatiFinanziariCantiere.saldo -= parseFloat(value);
        console.log(this.nuoviDatiFinanziariCantiere.saldo);
      }
    }
    this.nuoviDatiFinanziariCantiere.saldo += this.nuoviDatiFinanziariCantiere.creditiVsClienti;
    this.nuoviDatiFinanziariCantiere.saldo -= this.nuoviDatiFinanziariCantiere.debitiVsFornitori;
    this.nuoviDatiFinanziariCantiere.saldo = parseFloat(this.nuoviDatiFinanziariCantiere.saldo.toFixed(3)); // 3 cifre decimali
  ...
  isLastIndex(i: number): boolean {
    return this.nuovocant.valoriAggiuntivi.length === 0 || i === this.nuovocant.valoriAggiuntivi.length - 1;
  }
