...
import { ReportHSEService } from 'app/shared/services/data/reportHSE.service';
import { LoggingService, LogLevel } from 'app/shared/services/logging/logging.service';
import swal from 'sweetalert2';

@Component({
  selector: 'app-report-hse',
  templateUrl: './report-hse.component.html',
  styleUrls: ['./report-hse.component.scss', '../../../../assets/sass/libs/datatables.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ReportHSEComponent implements OnInit {
   ...
   */
  constructor(private service: CantieriService, private router: Router, private modalService: NgbModal, private files: ReportHSEService) {

  }

  ngOnInit(): void {
    // acquisisco i file relativi a questo cantiere
    this.service.getReportHSECantiere(this.targetCantiereId)
      .subscribe(
        (res) => {
          console.log('report HSE del cantiere ottenuti', res);
          this.fileget = res; this.tempData = res;
        },
        (err) => {
          LoggingService.log('errore nel get dei file del cantiere', LogLevel.warn, err)
        },
        () => { LoggingService.log('file del cantiere ottenuti', LogLevel.debug, this.fileget) }
      );
    console.log('id del cantiere HSE', this.targetCantiereId);
    this.filePostUrl = this.service.postReportHSECantiereUrl(this.targetCantiereId);
  }
  ...
  onDownload(target: _File) {
    ...
    this.files.downloadReport(target);
  ...
  onChange(file_id: number, event: any) {
    ...
    LoggingService.log('box selezionato ', LogLevel.debug, [file_id, event.target.checked]);
    ...
    LoggingService.log('array di checkbox report HSE cantiere', LogLevel.debug, this.checked);
  }
  ...
  onFileSelected(event) {
    ...
    if (file) {
      ...
      this.service.postReportHSECantiere(this.targetCantiereId, formData)
        .subscribe(
          (res) => {
