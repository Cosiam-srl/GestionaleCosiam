...
import { ScadenzeMezzo } from 'app/models/mezzo.model';
...
@Component({
  selector: 'app-mezzi-generic-table',
  templateUrl: './mezzi-generic-table.component.html',
  styleUrls: ['./mezzi-generic-table.component.scss', '../../../../assets/sass/libs/datatables.scss'],
  encapsulation: ViewEncapsulation.None
})
export class MezziGenericTableComponent implements OnInit {
  ...
  constructor(private spinner: NgxSpinnerService, private modalService: NgbModal, private personaleservice: PersonaleService, private mezziservice: MezziService, private _cdr: ChangeDetectorRef) {
  ...
  ngOnInit(): void {
    ...
    if (this.idMezzo) {
      this.mezziservice.getScadenzeMezzo(this.idMezzo)
        .subscribe(
          (res: ScadenzeMezzo[]) => {
            LoggingService.log('Scadenze ottenute', LogLevel.debug, res);
            res.forEach(element => {
              this.scadenzeMezzi.push(element);
            });
          },
          (err) => {
          ...
          () => {
            this.scadenzeMezzi = [...this.scadenzeMezzi];
            this.rows = this.scadenzeMezzi;
            this.tempData = this.scadenzeMezzi;
  ...
  modifyRecord(event: ScadenzeMezzo, content) {
  ...
  mostraScadenzeChiuse() {
    ...
    this.mezziservice.getScadenzeMezzoFiltered(this.idMezzo, this.showClosed, null, null)
      .subscribe(
        (res: ScadenzeMezzo[]) => {
          LoggingService.log('Scadenze ottenute dopo aver cliccato la checkbox ', LogLevel.debug, res);
          res.forEach(element => {
            this.scadenzeMezzi.unshift(element);
            this.spinner.hide();
          });
        },
        (err) => {
  ...
  updateStatoScandenza(event: ScadenzeMezzo) {
    this.spinner.show(undefined,
      {
        type: 'ball-fall',
        size: 'medium',
    ...
    this.mezziservice.changeScadenzaStatus(event.id, event.nota.nota.state)
      .subscribe(
        (res) => {
  ...
  CreaRecord(title: string) {
    ...
    this.nuovorecord.idMezzi = this.idMezzo;
    this.mezziservice.postScadenzaMezzo(this.nuovorecord)
      .subscribe(
        (res) => {
  ...
  postaAllegati() {
    this.mezziservice.postAttachmentScadenza(this.nuovorecord.id, this.formData)
      .subscribe(
        (res) => {
  ...
  UpdateScadenza(title: String) {
    ...
    this.mezziservice.updateScadenza(this.nuovorecord.id, {
      id: this.nuovorecord.id,
      performingDate: this.nuovorecord.performingDate,
      idMezzi: this.nuovorecord.idMezzi,
  ...
  refreshtable(res: ScadenzeMezzo) {
    this.scadenzeMezzi.unshift(this.nuovorecord);
    this.scadenzeMezzi = [...this.scadenzeMezzi];
    this.spinner.hide();
    ...
    this.nuovorecord = new ScadenzeMezzo();
    this.nuovorecord.nota = new TaggedNota();
    this.nuovorecord.nota.nota = new Note();
    this.rows = this.scadenzeMezzi;
    this.tempData = this.scadenzeMezzi;
  ...
  deleteAttachment(allegato: ScadenzeMezzo, index: number) {
    ...
    ret = swal.fire({
      ...
      if (result.value) {
        swal.fire({
          ...
          if (result.value) {
            this.mezziservice.deleteAttachmentScadenza(id)
              .subscribe(
                (res) => {
                ...
                () => {
                  ...
                  const y = this.scadenzeMezzi.findIndex(x => x.id == allegato.id);
                  LoggingService.log('l\'elemento è all\'indice', LogLevel.warn, y);
                  this.scadenzeMezzi[y].nota.listFile.splice(index, 1);
                  LoggingService.log('l\'elemento è stato eliminato', LogLevel.warn, this.scadenzeMezzi);
                  this.scadenzeMezzi = [...this.scadenzeMezzi];
  ...
  ConfirmText(description: string) {
    ...
    ret = swal.fire({
      ...
      if (result.value) {
        ...
        this.mezziservice.deleteScadenzeMezzo(this.checked)
          .subscribe(
            (res) => {
              LoggingService.log('ho eliminato', LogLevel.debug, res);
              for (const element of this.checked) {
                this.scadenzeMezzi.splice(
                  this.scadenzeMezzi.indexOf(
                    this.scadenzeMezzi.find(x => x.id == element)
                  ),
                  1
                )
              }
              ...
              swal.fire({
                icon: 'success',
                title: 'Fatto!',
                text: 'Gli elementi selezionati sono stati cancellati. La pa gina verrà ricaricata.',
                customClass: {
                  confirmButton: 'btn btn-success'
                },
              }).then(() => {
  ...
  downloadAttachment(file: _File) {
    this.mezziservice.downloadAttachment(file);
  }
