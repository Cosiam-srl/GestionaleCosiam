...
export class BeniEServiziComponent implements OnInit {
  ...
  constructor(
    ...
    private servizifornitore: SupplierService
  ) {}

  ngOnInit(): void {
    this.servizifornitore.getServiziFornitore(this.targetFornitoreId).subscribe(
      (res) => {
  ...
  convertStringToDecimal(number: number): number {
  ...
  aggiungiServizioFornitore() {
    ...
    this.nuovoservizio.pricePerUM = this.convertStringToDecimal(
      this.nuovoservizio.pricePerUM
    );
    this.servizifornitore.postServizioFornitore(this.nuovoservizio).subscribe(
      (res) => {
        this.nuovoservizio.id = res.id;
        LoggingService.log(
          "nuovo prezziario postato con successo",
          LogLevel.debug,
          res
        );
        swal.fire({
  ...
  updateServizioFornitore() {
    LoggingService.log(
      "sto per fare l'upload di questo servizio",
      LogLevel.debug,
      this.nuovoservizio
    );
    this.nuovoservizio.pricePerUM = this.convertStringToDecimal(
    ...
    this.servizifornitore
      .updateServizioFornitore(this.nuovoservizio, this.nuovoservizio.id)
      .subscribe(
        (res) => {
          ...
          const pos = this.servizi.indexOf(
            this.servizi.find((x) => x.id === this.nuovoservizio.id)
          );
    ...
    ret = swal
      ...
      .then((result) => {
        if (result.value) {
          ...
          LoggingService.log(
            "array di servizi da eliminare",
            LogLevel.debug,
            this.checked
          );
          // eliminiamo le note selezionate
          this.servizifornitore.deleteServiziFornitore(this.checked).subscribe(
            (res) => {
